...
def train_model(X_train, y_train):
    ...
    # Initialize and train the K nearest neighbour
    ...
    return gnb, clf, knn, rf

def save_model(model, filename='disease_prediction_model.sav'):
    # Save the trained model to a file
    joblib.dump(model, filename)

def load_model(filename='disease_prediction_model.sav'):
    # Load the trained model from a file
    return joblib.load(filename)

def predict_disease(model, symptoms):
    # Predict disease based on symptoms
    return diseases[model.predict([symptoms])[0]]

# Main function for training and saving the model
def main():
    # Preprocess the training data
    X, y = preprocess_data("Training.csv")
    
    # Train the models
    gnb, clf, knn, rf = train_model(X, y)
    
    # Save the trained models
    save_model(gnb, filename='naive_bayes_model.sav')
    save_model(clf, filename='decision_tree_model.sav')
    save_model(knn, filename='kNearest_model.sav')
    save_model(rf, filename='random_forest_model.sav')
